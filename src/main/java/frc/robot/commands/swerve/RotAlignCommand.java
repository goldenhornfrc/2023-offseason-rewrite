...
public class RotAlignCommand extends CommandBase {
  ...
  public void initialize() {
    output = 0.0;
    m_swerve.getRotController().setTolerance(Constants.SwerveConstants.pidConstants.rotControllerTolerance);
  }

  // Called every time the scheduler runs while the command is scheduled.
  @Override
  public void execute() {
    output = m_swerve.getRotController().calculate(m_swerve.getHeadingRotation().getDegrees(), Robot.getAlliance() == Alliance.Blue ? 0.0 : 180.0);
